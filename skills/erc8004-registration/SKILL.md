...
description: Register AI agents on Ethereum mainnet using ERC-8004 (Trustless Agents). Use when the agent needs to register its identity on-chain, create an agent profile, claim an agent NFT, set up agent reputation, or make agents discoverable. Handles IPFS upload and on-chain registration.
metadata:
  {"openclaw": {"emoji": "🪪", "requires": {"env": ["AGENT_PRIVATE_KEY"]}, "primaryEnv": "AGENT_PRIVATE_KEY"}}
---
...
## Agent Safety Monitor Registration Data

When registering THIS agent, use these values:
- **Name:** Agent Safety Monitor
- **Description:** Autonomous watchdog agent monitoring AI agents on Base blockchain. Calculates real-time reputation scores, detects malicious behavior, and enforces accountability through onchain mechanisms.
- **Services:** reputation-query endpoint, X handle (@AgentSafetyBase)
- **supportedTrust:** ["reputation"]
- **active:** true

## IMPORTANT: Script Dependencies

The bash scripts in `{baseDir}/scripts/` reference `~/clawd/skills/bankr/scripts/bankr.sh` for transaction submission. This is Clawd's wallet skill and will NOT work in our setup. For actual registration, use either:
1. **Option 1 (recommended):** The 8004.org frontend at https://www.8004.org
2. **Option 2:** The Agent0 SDK (`agent0-sdk`) with our agent's private key
3. **Option 3:** The `register-onchain.sh` script adapted to use ethers.js instead of Bankr
